...
const MAX_RUNTIME_MS = positiveIntFromEnv("PI_TASK_MAX_RUNTIME_MS", 10 * 60_000);
...
async function runBackgroundBatch(
  ...
  try {
    const results = await mapLimit(params.tasks, MAX_CONCURRENCY, async (task) => {
      ...
      try {
        const result = await runSubtask(ctxCwd, params.agent, params.context, task, controller.signal, update);
