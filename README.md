...
| --- | --- |
| **`@swiftengineer/pi-harness`** _(this repo)_ | The default agent harness: compact system prompt, tool policy, task/subagents, `search`, `ls`/`find`, AST tools, `todo_write`, `ask`, and last-used-model defaults. |
...
2. Registers the harness and the plugins (`pi-mcp-adapter`, `pi-web-access`, `pi-powerline-footer`) via `pi install`.
