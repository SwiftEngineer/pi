 ...
 *
 * Adapted from extensions/subagent-view/strip.ts. Pure width-bounded rendering
 * logic ports 1:1. The ONLY adaptation is the value import of
 * `truncateToWidth`/`visibleWidth`: under PiJS these are not provided by
 * `@earendil-works/pi-tui`, so they come from `../_shared/textwidth.ts` (§5.4).
 * `import type { Theme }` is erased at runtime (same pattern as registry.ts).
 *
 ...
 * once: status → glyph shape + color, selected → inverse video, unread →
 * underline. Status stays distinguishable with color off (glyph shapes differ),
 * so it is colorblind-safe. Pure and width-bounded, so it verifies headlessly.
 */
...
function inverse(styled: string, theme: Theme, ascii: boolean, glyph: Glyph, _channel: StripChannel): string {
...
/**
 * The controls hint. ⚠ Overlay-only port: ONLY the channel-switch shortcuts are
 * actually wired (`alt+]`/`alt+[`, aliases `alt+s`/`alt+a` via registerShortcut).
 * The split-pane `PgUp/PgDn scroll`, `Esc → main`, and `alt+l live` keys are NOT
 * installed under the overlay, so they are deliberately not advertised here
 * (advertising dead keys would mislead the user).
 */
function controlsLine(width: number, theme: Theme): string {
  return truncateToWidth(theme.fg("dim", "alt+]/[ switch"), width, "…", true);
}
