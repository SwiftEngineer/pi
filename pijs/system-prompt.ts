 ...
 *
 * Adapted from extensions/system-prompt.ts. The COMPACT_SYSTEM_PROMPT string
 * ports verbatim; the only adaptation is dropping the
 * `@earendil-works/pi-coding-agent` type import (the `pi` object is duck-typed
 * here). One `before_agent_start` hook returns `{ systemPrompt }` — the
 * system-prompt injection event in the JS dispatcher (not `session_start`).
 * No deps, no schema, no capabilities.
 */
...
export default function (pi: {
  on: (event: "before_agent_start", handler: () => { systemPrompt: string }) => void;
}) {
